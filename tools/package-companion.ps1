...
$zip = Join-Path $root 'MistriaTracker-Companion-0.1.5.zip'
if (-not (Test-Path -LiteralPath (Join-Path $source 'manifest.json'))) { throw 'Companion manifest not found.' }
if (-not (Test-Path -LiteralPath (Join-Path $source 'gml\MistriaTrackerCompanion.gml'))) { throw 'Companion GML not found.' }
